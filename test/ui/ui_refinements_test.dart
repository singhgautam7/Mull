...
import 'package:mull/features/collections/add_to_shelf_screen.dart';
...
import 'package:mull/shared/widgets/states.dart';
...
void main() {
  ...
  setUpAll(() async {
    ...
    await (FontLoader(
      'Instrument Sans',
    )..addFont(Future<ByteData>.value(font))).load();
  });
  ...
  testWidgets('AppIconButton displays AppTooltip on long press', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets(
    'AppSnackbar shows timed top snackbar with linear progress indicator and close button',
            ...
            body: Builder(
              builder: (BuildContext context) => Center(
                child: ElevatedButton(
                  onPressed: () =>
                      AppSnackbar.info(context, 'Test snackbar message'),
  ...
  testWidgets(
    'WordCard action row contains Open full entry, Bookmark, Add note, and More',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: AppTheme.of(ThemeFamily.byId('mull'), Tone.light),
          home: Scaffold(
            body: WordCard(
              ...
              examples: const <String>[
                'Collecting bus tickets and other ephemera.',
              ],
  ...
  testWidgets(
    'AddToShelfScreen (HANDOFF 26) lists the shelf ticked, and a tap toggles',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{});
      final UserRepository userRepo = UserRepository(userDb);
      await userRepo.ensureSystemCollections();
      await userRepo.addToCollection(UserRepository.readingSlug, ephemera.key);
      ...
      await tester.runAsync(() async {
        ...
        await tester.pumpWidget(
          ProviderScope(
            ...
            child: MaterialApp(
              theme: AppTheme.of(ThemeFamily.byId('mull'), Tone.light),
              home: const AddToShelfScreen(
                slug: UserRepository.readingSlug,
                phrases: false,
              ),
            ),
          ),
        ...
        // Before a query: the shelf itself, every row ticked.
        expect(find.text('ON THIS SHELF · 1'), findsOneWidget);
        expect(find.text('ephemera'), findsOneWidget);
        expect(find.byIcon(Icons.check_rounded), findsOneWidget);
        expect(find.text('1 on the shelf'), findsOneWidget);

        // Tapping the row takes it off, with Undo, not a duplicate warning.
        await tester.tap(find.text('ephemera'));
        for (int i = 0; i < 5; i++) {
          await Future<void>.delayed(const Duration(milliseconds: 20));
          await tester.pump();
        }
        expect(find.text('Taken off the shelf'), findsOneWidget);
        expect(await userRepo.collectionWordKeys(UserRepository.readingSlug), isEmpty);

        // Searching shows the same word with a plus, and a tap adds silently.
        ...
        expect(await userRepo.collectionWordKeys(UserRepository.readingSlug), <String>[ephemera.key]);
        expect(find.text('1 added just now'), findsOneWidget);
  ...
  testWidgets(
    'AppButton of type outlined has surface background and accent border',
    (WidgetTester tester) async {
  ...
  testWidgets(
    'SettingsScreen pronunciation speed row opens bottom sheet with slider',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboarded': true,
      });
  ...
  testWidgets(
    'MoreScreen shows Data row under Your data below Permissions without Export',
    (WidgetTester tester) async {
      await tester.runAsync(() async {
        SharedPreferences.setMockInitialValues(<String, Object>{
          'onboarded': true,
  ...
  testWidgets('DataScreen shows import and export, then clear and reset', (
    WidgetTester tester,
  ) async {
    await tester.runAsync(() async {
      SharedPreferences.setMockInitialValues(<String, Object>{});
      ...
      expect(find.text('JSON · everything but the dictionary'), findsOneWidget);
      expect(find.text('Import app data'), findsOneWidget);
      expect(find.text('CLEAR AND RESET'), findsOneWidget);
  ...
  testWidgets('TopicCard formats metrics as "187 · 3 seen" with accent color', (
    ...
    final RichText richText = tester.widget<RichText>(
      find.byWidgetPredicate(
        (Widget w) => w is RichText && w.text.toPlainText().contains('187'),
      ),
    );
  ...
  testWidgets(
    'PillChip displays badge count only when selected with showCountWhenSelectedOnly',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: AppTheme.of(ThemeFamily.byId('mull'), Tone.light),
  ...
  testWidgets(
    'CollectionsScreen holds card-shaped placeholders until the user streams land',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{'onboarded': true});
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      final DictionaryDb topics = fakeDictionary(
        words: <FakeWord>[ephemera],
        collections: <String, List<String>>{
          'nature': <String>[ephemera.key],
          'core': <String>[ephemera.key],
        },
        details: <String, (String, String, String)>{
          'core': ('band', 'Easy Words', 'Common.'),
        },
      );
      ...
      await tester.runAsync(() async {
        await tester.pumpWidget(
          ProviderScope(
            overrides: <Override>[
              prefsProvider.overrideWithValue(prefs),
              userDatabaseProvider.overrideWithValue(userDb),
              dictProvider.overrideWithValue(topics),
            ],
            child: MaterialApp(
              theme: AppTheme.of(ThemeFamily.byId('mull'), Tone.light),
        ...
        // First frame: the dictionary's shelves are known, so their slots are
        // drawn at the real card and row shapes, but as placeholders.
        expect(find.byType(SkeletonCard), findsOneWidget);
        expect(find.byType(SkeletonRow), findsNWidgets(3));
        expect(find.byType(TopicCard), findsNothing);

        for (int i = 0; i < 10 && find.byType(SkeletonCard).evaluate().isNotEmpty; i++) {
          await Future<void>.delayed(const Duration(milliseconds: 20));
          await tester.pump();
        }
        ...
        expect(find.byType(TopicCard), findsOneWidget);
        expect(find.text('Easy Words'), findsOneWidget);
        await tester.pumpWidget(const SizedBox());
      });
    },
  );
  ...
  testWidgets(
    'CollectionsScreen shows Shelves title and Words/Phrases filters',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboarded': true,
      });
  ...
  testWidgets(
    'CollectionScreen header has + button opening Add words/phrases menu',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboarded': true,
      });
  ...
  testWidgets(
    'CollectionScreen shows floating Mull button and 6-segment overflow menu',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboarded': true,
      });
  ...
  testWidgets(
    'CollectionScreen on user shelf shows Delete in red in overflow menu',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboarded': true,
      });
      final SharedPreferences prefs = await SharedPreferences.getInstance();
      final UserRepository userRepo = UserRepository(userDb);
      final String userSlug = await userRepo.createCollection(
        'My Custom Shelf',
      );
  ...
  testWidgets(
    'CollectionsScreen overflow menu has How to use and opens sheet',
    (WidgetTester tester) async {
      SharedPreferences.setMockInitialValues(<String, Object>{
        'onboarded': true,
      });
